...
class RNASeq (Workflow):
    ...
    def define_parameters(self, parameters_section=None):
        ...
        self.add_parameter ( "genome_dir", "Path to the directory containing the STAR index of the human genome.", required=True )
        self.add_input_file( "genome_annot", "Path to the annotation of the human genome (format: GTF).", required=True )
        self.add_input_file( "genome_seq", "Path to the sequences of the human genome (format: FASTA).", required=True )
        self.add_input_file( "genome_SNV", "Path to the variants of the human genome (format: VCF).", required=True )
        self.add_parameter( "transcriptome_idx", "Path to the salmon index for the human transcriptome (format: salmon index)." )
    ...
    def process(self):
        ...
        # SNV detection
        #~ gatk_prepro = self.add_component( "GATKPreprocess", [star.aln_files, self.genome_seq, False] )
        ...
        #~ variant_annotation = self.add_component( "VEP", [variant_filtering.marked_SNV_files, "homo_sapiens", "GRCh38"] )
