...
class GATKFilter (Component):
    ...
    def define_parameters(self, SNV_files, reference_seq, filters=None, discard=True, memory=10):
        ...
        # Input files
        self.add_input_file_list( "SNV_files", "********************** (format: VCF).", default=SNV_files, required=True )
    ...
    def process(self):
        ...
        cmd = "java -Xmx" + str(self.memory) + "g -jar " + self.get_exec_path("GATK") + \
              ...
              " --out $2" + \
              " 2> $3"
        gatk_mark_filtered = ShellFunction( cmd, cmd_format='{EXE} {IN} {OUT}' )
        MultiMap( gatk_mark_filtered, inputs=[self.SNV_files], outputs=[self.marked_SNV_files, self.marked_stderr], includes=self.reference_seq )
        ...
        if self.discard:
            cmd = "java -Xmx" + str(self.memory) + "g -jar " + self.get_exec_path("GATK") + \
                  ...
                  " --out $2" + \
                  " 2> $3"
            gatk_filter = ShellFunction( cmd, cmd_format='{EXE} {IN} {OUT}' )
            MultiMap( gatk_filter, inputs=[self.marked_SNV_files], outputs=[self.filtered_SNV_files, self.filtered_stderr], includes=self.reference_seq )
