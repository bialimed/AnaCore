...
def gatkPreproWrapper(jvm_options, picard_path, gatk_path, sequencer, is_RNA, reference_genome, in_aln_file, out_aln_file, stderr):
    """
    """
    import os
    import subprocess
...
class GATKPreprocess (Component):
    ...
    #~ def define_parameters(self, aln_files, genome_seq, is_RNA=False, sequencer="ILLUMINA", samples_names=None, libraries_names=None, platform_units=None, memory=30):
    def define_parameters(self, aln_files, genome_seq, is_RNA=False, sequencer="ILLUMINA", memory=20):
        ...
        #~ self.add_parameter_list( "libraries_names", "************************************", default=libraries_names )
        self.add_parameter( "memory", "The memory size to allocate (in GigaBytes).", type="int", default=memory )
        #~ self.add_parameter_list( "platform_units", "************************************", default=platform_units )
        #~ self.add_parameter_list( "samples_names", "************************************", default=samples_names )
    ...
    def process(self):
        cmd_fmt = '{EXE}' + \
                  ...
                  ' "' + self.genome_seq + '"' + \
                  ' {IN} {OUT}'
        gatk_prepro = PythonFunction( gatkPreproWrapper, cmd_format=cmd_fmt )
        MultiMap( gatk_prepro, inputs=[self.aln_files], outputs=[self.cleaned_files, self.stderr], includes=[self.genome_seq] )
