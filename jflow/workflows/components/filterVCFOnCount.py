...
class FilterVCFOnCount (Component):
    ...
    def define_parameters(self, in_variants, min_AF=0.02, min_DP=120, mode="tag"):
        # Parameters
        self.add_parameter( 'min_AF', 'Under this allele frequency the variant does not pass the "lowAF" filter.', type=float, default=min_AF )
        self.add_parameter( 'min_DP', 'Under this depth in ******************** the variant does not pass the "lowAF" filter.', type=int, default=min_DP )
        ...
        self.add_output_file_list( "out_variants", "****************** (format: VCF).", pattern='{basename_woext}.vcf', items=self.in_variants )
