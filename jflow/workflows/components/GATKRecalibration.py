...
#~ def gatkRecalibWrapper(jvm_options, gatk_path, reference_genome, reference_variants, in_aln_file, out_aln_file, stderr):
    #~ """
    #~ """
    #~ import subprocess

    #~ # Recalibration detection
	#~ recalibDetection_out = out_aln_file + "_recalibDetection.txt" )
    #~ cmd = "java " + jvm_options + " -jar " + gatk_path + \
          #~ " --analysis_type BaseRecalibrator" + \
          #~ " --reference_sequence " + reference_genome + \
          #~ " --knownSites " + reference_variants + \
          #~ " --input_file " + in_aln_file + \
          #~ " --out " + recalibDetection_out + \
          #~ " 2> " + stderr
