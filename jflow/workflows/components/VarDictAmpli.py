...
class VarDictAmpli (Component):
    ...
    def define_parameters(self, in_genome_seq, in_regions, in_aln, min_AF=0.02):
        # Parameters
        self.add_parameter( "min_AF", "********", type=float, default=min_AF, required=True )

        # Input files
        self.add_input_file_list( "in_aln", "***** (format: BAM)", default=in_aln, required=True )
        self.add_input_file( "in_genome_seq", "***** (format: fasta)", default=in_genome_seq, required=True )  
        ...
        self.add_output_file_list( "out_variants", "******************", pattern='{basename_woext}.vcf', items=self.in_aln )
        self.add_output_file_list( "calling_stderr", "The stderr file of the calling step.", pattern='{basename_woext}_calling.stderr', items=self.in_aln )
        self.add_output_file_list( "filter_stderr", "The stderr file of the filter step.", pattern='{basename_woext}_filter.stderr', items=self.in_aln )
        self.add_output_file_list( "convert_stderr", "The stderr file of the conversion step.", pattern='{basename_woext}_convert.stderr', items=self.in_aln )
