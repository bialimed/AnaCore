...
class GATKHaplotypeCaller (Component):
    ...
    def define_parameters(self, aln_files, genome_seq, genome_SNV=None, intervals=None, std_call_conf=30, std_emit_conf=30, memory=30):
        ...
        self.add_output_file_list( "stderr", "The HaplotypeCaller error files", pattern='{basename_woext}.stderr', items=self.aln_files )
    ...
    def process(self):
        ...
        cmd = "java -Xmx" + str(self.memory) + "g -jar " + self.get_exec_path("GATK") + \
              ...
              " --out $2" + \
              " 2> $3"
        gatk_haplotypecaller = ShellFunction( cmd, cmd_format='{EXE} {IN} {OUT}' )
        MultiMap( gatk_haplotypecaller, inputs=[self.aln_files], outputs=[self.SNV_files, self.stderr], includes=includes_files )
