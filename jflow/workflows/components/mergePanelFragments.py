...
class MergeOverlappingFragments (Component):

    def define_parameters(self, in_regions, in_variants, in_aln):
    ...
    def process(self):
		cmd = self.get_exec_path("mergePanelFragments.py") + \
		...
		MultiMap(merge_fct, inputs=[self.in_variants], outputs=[self.out_variants, self.stderr], includes=[self.in_regions] )
