...
class SamtoolsFlagStat (Component):
    ...
    def process(self):
        ...
        if self.search_dupl:
            ...
            markduplicates = ShellFunction( 'java -Xmx6g -jar ' + self.get_exec_path("picardtools") + ' MarkDuplicates INPUT=$1 METRICS_FILE=$2 OUTPUT=$3' + self.duplication_options + ' 2> $4', 
                                            cmd_format='{EXE} {IN} {OUT}' )
            MultiMap( markduplicates, inputs=self.aln_files, outputs=[self.duplication_files, tmp_bam, self.duplication_stderr] )
             
        # Alignment summary
        flagstat = ShellFunction( self.get_exec_path("samtools") + " view -F0x0100 -bh $1 | " + self.get_exec_path("samtools") + " flagstat - > $2 2> $3", 
                                  cmd_format='{EXE} {IN} {OUT}' )
