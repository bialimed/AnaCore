...
class ReadsStat(Component):
    ...
    def process(self):
        cmd = self.get_exec_path("fastqc") + \
            ...
            ' $1 > $2 2> $3'
        fastqc_fct = ShellFunction(cmd, cmd_format='{EXE} {IN} {OUT}')
        MultiMap(fastqc_fct, inputs=self.input_files, outputs=[self.stdouts, self.stderrs])
