...
if __name__ == "__main__":
    ...
    group_output.add_argument( '-o', '--output-amplicons', default="amplicons.tsv", help='******************************** (format: TSV). [Default: %(default)s]' )
        ...
        for record in FH_seq:
            ...
            if chrom in ampl_by_chr:
                for area in ampl_by_chr[chrom]:
                    area.annot = {
                        "sequence": record.string[area.start-1:area.end],
                        "up_primer": record.string[area.start-1:area.thickStart-1],
                        "down_primer": record.string[area.thickEnd:area.end]
                    }
                    if area.strand == "-":
                        area.annot["sequence"] = revcom(area.annot["sequence"])
                        aux_up_primer = area.annot["up_primer"]
                        area.annot["up_primer"] = revcom(area.annot["down_primer"])
                        area.annot["down_primer"] = aux_up_primer
                    else:
                        area.annot["down_primer"] = revcom(area.annot["down_primer"])
    with open(args.output_amplicons, "w") as FH_out:
        for chrom in sorted(ampl_by_chr):
            for area in ampl_by_chr[chrom]:
                FH_out.write(
                    area.name + "\t" + area.annot["up_primer"] + "\t" + area.annot["down_primer"] + "\n"
                )
