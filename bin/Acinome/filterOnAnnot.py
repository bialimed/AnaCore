...
#
########################################################################
def filterByConsequences( allele_record, VEP_alt, valid_consequences ):
    removed_idx = list()
        ...
        if VEP_alt == annot["Allele"]:
            ...
            for current_csq in consequences:
                if current_csq in valid_consequences:
                    is_filtered = False
        if is_filtered:
            removed_idx.append( annot_idx )
    for idx in sorted(removed_idx)[::-1]:
        del( allele_record.info["CSQ"][idx] )
...
if __name__ == "__main__":
    # Manage parameters
    parser = argparse.ArgumentParser( description='Filter VCF on annotation criteria.' )
    ...
    group_input.add_argument( '-i', '--variant-file', required=True, help='The path to the file file containing variants and annotated with VEP v88+ (format: VCF).' )
    group_output = parser.add_argument_group( 'Outputs' ) # Outputs
    group_output.add_argument( '-o', '--output-file', required=True, help='The path to the file containing the filtered variants and annotations (format: VCF).')
    args = parser.parse_args()

    # Process
    with open(args.output_file, "w") as FH_out:
        # Filter variant file
        with open(args.variant_file) as FH_vcf:
        ...
        with VEPVCFIO(args.variant_file) as FH_in:
            for record in FH_in:
                ...
                for alt_idx, alt in enumerate(record.alt):
                    ...
                    # Filter consequences
                    filterByConsequences( alt_record, VEP_alt[alt_idx], args.kept_consequences )
                    # Filter the variant allele
                    if not is_polymophism and len(alt_record.info["CSQ"]) > 0:
