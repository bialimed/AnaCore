...
from anacore.region import consolidate
...
if __name__ == "__main__":
    ...
    if bed_nb_col > 4:
        log.warning("The BED file {} contains {} columns all the columns after the 4th willbe removed on output.".format(args.input_regions, bed_nb_col))
    regions = getAreas(args.input_regions)

    # Add padding
    for curr_region in regions:
        curr_region.start = max(1, curr_region.start - args.padding_size)
        curr_region.end = curr_region.start + args.padding_size  # The max reference size is not managed

    # Merge overlapping
    consolidated_regions = consolidate(regions, args.merge_contiguous, True)

    # Write output
    with BEDIO(args.output_regions, "w") as FH_out:
        for curr_region in consolidated_regions:
            if args.concatenate_names and "merge_traceback" in curr_region.annot:
                curr_region.name = ";".join(curr_region.annot["merge_traceback"])
